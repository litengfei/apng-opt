...
public class CRC32Replace implements Runnable {
    ...
    private static final int XORNOT = 0xFFFFFFFF;
    static int crc32c_table[] =
    ...
    private void testCrc() {
        ...
        crc32.reset();
        buf[0] = 'f';
        crc32.update(buf, 0, 1);
        int x1 = (int) crc32.getValue();

        crc32.reset();
        buf[0] = 'I';
        crc32.update(buf, 0, 1);
        int y1 = (int) crc32.getValue();
        int r1 = x1 ^ y1;

        System.out.println(String.format(
                "x1: %H, y1: %H, r1: %H, t1: %H, t2: %H  - %s",
                x1, y1,
                r1,
                x1 ^ XORNOT ^ 0x00FFFFFF, y1 ^ XORNOT ^ 0x00FFFFFF,
                findIndexPair(x1 ^ y1)
        ));
        ...
        buf[0] = 'f';
        buf[1] = '2';
        crc32.update(buf, 0, 2);
        int x2 = (int) crc32.getValue();

        crc32.reset();
        buf[0] = 'I';
        buf[1] = '2';
        crc32.update(buf, 0, 2);
        int y2 = (int) crc32.getValue();
        int r2 = x2 ^ y2;
        System.out.println(String.format(
                "x2: %H, y2: %H, r2: %H, t1: %H, t2: %H  - %s",
                x2, y2,
                r2,
                x2 ^ XORNOT ^ 0x00FFFFFF, y2 ^ XORNOT ^ 0x00FFFFFF,
                findIndexPair(r2 ^ (r1 >> 8))
        ));
        System.out.println(String.format(
                "x2: %H, y2: %H, r2: %H",
                ((x1 ^ XORNOT) >> 8) ^ crc32_table[(x1 ^ XORNOT ^ '2') & 0xFF],
                ((y1 ^ XORNOT) >> 8) ^ crc32_table[(y1 ^ XORNOT ^ '2') & 0xFF],
                r2
        ));
    }
    ...
    private void genXorCodeTable() {
        ...
        for (int i = 0; i < data.length - 4; i++) {
            ...
            System.out.println(String.format("%d: %H  - %s", i, x, findIndexPair((pre >> 8) ^ x)));
            pre = x;
        }
    }

    private String findIndexPair(int x) {
        String pair = "";
        for (int i = 0; i < 256; i++) {
            for (int j = 0; j < 256; j++) {
                if ((crc32_table[i] ^ crc32_table[j]) == x) {
                    pair += String.format("[%d,%d]", i, j);
                }
            }
        }
        return pair;
